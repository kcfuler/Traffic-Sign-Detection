...
def constrastLimit(image):
    ...
    channels = list(cv2.split(img_hist_equalized))  # 转换为list使其可变
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    ...
    if sign is not None:
        ...
        cv2.imwrite(os.path.join('output', str(count)+'_'+text+'.png'), sign)
...
def main(args):
    ...
    out = cv2.VideoWriter(os.path.join(
        'output', 'output.avi'), fourcc, fps, (640, 480))
    ...
    coordinates = []
    position = []
    file = open(os.path.join('output', 'Output.txt'), "w")
    while True:
